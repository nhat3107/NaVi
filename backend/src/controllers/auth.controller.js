  ...
  try {
    ...
    if(existingOTP) {
      return res.status(400).json({
        message: "Email verification already in progress. Please check your email or request a new OTP. Go login to continue your verification process.",
    ...
    res.status(201).json({ 
      success: true, 
      message: "Please check your email for OTP verification to complete your registration.",
      requiresOTPVerification: true,
      email: email
    });
  } catch (error) {
  ...
  try {
    ...
    if(!user) {
      ...
      if (pendingOTP) {
        ...
        const isPasswordValid = await bcryptjs.compare(password, pendingOTP.passwordHash);
        if (isPasswordValid) {
          return res.status(400).json({
...
export const resendOTP = async (req, res) => {
  ...
  try {
    ...
    if (timeElapsed < 60 * 1000) { // 1 minute
