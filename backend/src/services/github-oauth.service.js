...
import { getUserByEmail, createUser, getUserByGithubId, linkGithubAccount, updateUser } from "./user.service.js";
...
export const exchangeCodeForToken = async (code) => {
  try {
    ...
    if (response.data.error) {
      const errorMessage = response.data.error === 'bad_verification_code' 
...
export const getGithubUserInfo = async (accessToken) => {
  ...
  try {
    ...
    return await processGithubUserData(userResponse.data, accessToken, headers, 'Bearer');
  } catch (error) {
    // Fallback to token format if Bearer fails
    if (error.response?.status === 401) {
      try {
        const userResponse = await axios.get(GITHUB_USER_URL, {
          headers: { ...headers, 'Authorization': `token ${accessToken}`, 'Accept': 'application/vnd.github.v3+json' }
        });
        
        return await processGithubUserData(userResponse.data, accessToken, { ...headers, 'Accept': 'application/vnd.github.v3+json' }, 'token');
      } catch (retryError) {
        throw new Error('Failed to get user info from GitHub: ' + retryError.message);
      }
    }
...
const processGithubUserData = async (userData, accessToken, headers, authType) => {
...
export const handleGithubAuth = async (githubUser) => {
  try {
    ...
    // 3. Create new user
    if (!githubUser.email) {
      throw new Error('GitHub account must have a public email or verified email to create account');
    }
    ...
    const newUser = await createUser({
      ...
      avatarUrl: githubUser.avatar_url || "https://cloudanary.s3.ap-southeast-1.amazonaws.com/basic-avatar.jpg",
      bio: githubUser.bio || "",
      isOnBoarded: false
    });
